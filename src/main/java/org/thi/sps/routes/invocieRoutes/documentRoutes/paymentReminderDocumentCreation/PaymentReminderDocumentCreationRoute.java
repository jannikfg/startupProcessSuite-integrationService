...
import org.thi.sps.routes.invocieRoutes.documentRoutes.objects.PaymentForDocumentService;
import org.thi.sps.routes.invocieRoutes.documentRoutes.objects.ReminderForDocumentService;
  ...
  public void configure() throws Exception {
    ...
    from("kafka:" + TOPIC_NAME + "?brokers=" + redpandaUrl)
        ...
        .process(exchange -> {
          ...
          List<Map<String, Object>> paymentsData = (List<Map<String, Object>>) invoiceData.get("payments");
          if (paymentsData != null) {
            List<PaymentForDocumentService> payments = new ArrayList<>();
            for (Map<String, Object> paymentData : paymentsData) {
              PaymentForDocumentService payment = new PaymentForDocumentService();
              payment.setId(Long.parseLong(paymentData.get("id").toString()));
              payment.setInvoiceId((String) paymentData.get("invoiceId"));
              payment.setPaymentDate(LocalDate.parse((String) paymentData.get("paymentDate")));
              payment.setAmount(Double.parseDouble(paymentData.get("amount").toString()));
              payment.setMethod((String) paymentData.get("method"));
              payment.setReference((String) paymentData.get("reference"));

              payments.add(payment);
            }
            invoiceForDoc.setPayments(payments);  // Setze die Payment-Liste im Invoice-Objekt
          }

          List<Map<String, Object>> reminderData = (List<Map<String, Object>>) invoiceData.get("reminders");
          log.info("reminderData: " + reminderData);
          if (reminderData != null) {
            List<ReminderForDocumentService> reminders = new ArrayList<>();
            for (Map<String, Object> reminderItemData : reminderData) {
              ReminderForDocumentService reminder = new ReminderForDocumentService();
              reminder.setId(reminderItemData.get("id").toString());
              reminder.setReminderLevel(reminderItemData.get("reminderLevel").toString());
              reminder.setReminderDate(LocalDate.parse(reminderItemData.get("reminderDate").toString()));
              reminder.setDueInDays(Integer.parseInt(reminderItemData.get("dueInDays").toString()));
              reminder.setTotalOutstanding(Double.parseDouble(reminderItemData.get("totalOutstanding").toString()));

              reminders.add(reminder);
              log.info("reminder: " + reminder);
            }
            invoiceForDoc.setReminders(reminders);
          }
          ...
          DunningCreationRequest dunningCreationRequest = new DunningCreationRequest();
          dunningCreationRequest.setInvoice(invoiceForDoc);

          ClientForDocumentService client = exchange.getIn()
              .getBody(ClientForDocumentService.class);
          String reminderLevel = invoiceForDoc.getReminders().get(invoiceForDoc.getReminders().size() - 1).getReminderLevel();
          dunningCreationRequest.setClient(client);
          dunningCreationRequest.setDunningId(invoiceForDoc.getId() + "_" + reminderLevel + "_" + LocalDate.now());
          dunningCreationRequest.setReminderLevel(reminderLevel);
          dunningCreationRequest.setReminderDate(LocalDate.now());
          dunningCreationRequest.setDueInDays(invoiceForDoc.getReminders().get(invoiceForDoc.getReminders().size() - 1).getDueInDays());


          log.info("DunningCreationRequest: " + dunningCreationRequest);
          exchange.getIn().setBody(dunningCreationRequest);
        })
